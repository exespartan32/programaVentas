...
$(".tablaVentas tbody").on("click", "button.agregarProducto", function () {
	...
	$.ajax({
		...
		success: function (respuesta) {
			...
			var ContainerPrices = ($("#ContainerPrices").find(".row").length) + 1
			arrayData.push(ContainerPrices)

			var numProd = "productoNº" + arrayData.at(-1)


			var moneda = $('#monedaCambio').val()
			var ContainerPricesLengt = $("#ContainerPrices").find(".productN").length + 1
			...
			if(moneda == null){
				...
				if (moneda == 'USD') {
					contenidoHTML = completarHTML(dolarCom, idProducto, descripcion, stock, precio)
				}
				if (moneda == 'EUR') {
					contenidoHTML = completarHTML(euroCom, idProducto, descripcion, stock, precio)
				}
				if (moneda == 'JPY') {
					contenidoHTML = completarHTML(yenCom, idProducto, descripcion, stock, precio)
				}
				if (moneda == 'CNY') {
					contenidoHTML = completarHTML(yuanCom, idProducto, descripcion, stock, precio)
				}
				if (moneda == 'RUB') {
					contenidoHTML = completarHTML(rupiaCom, idProducto, descripcion, stock, precio)
				}
			}
			//console.log(contenidoHTML)
			$(".nuevoProducto").append(contenidoHTML)
			...
			<div class="row" style="padding:5px 15px">
				...
				<div class="col-xs-6" style="padding-right:0px">
					<div class="input-group">
						...
						<input type="text" class="form-control nuevaDescripcionProducto" idproducto="${idProducto}" name="agregarProducto"
						value="${descripcion}" readonly required>
						<input type="hidden" class="idProducto" value="${idProducto}">
					</div>
				</div>
				...
				<div class="col-xs-3">
					...
					min="1" value="1" stock="${stock}" nuevostock="${Number(stock - 1)}" required>
				</div>
		
				<!-- Precio del producto -->
				<div class="col-xs-3 ingresoPrecio" style="padding-left:0px">
					<div class="input-group">
						<span class="input-group-addon productN" id="${numProd}">
							<span class="material-symbols-outlined">attach_money</span>
						</span>
						<input type="text" class="form-control nuevoPrecioProducto" onchange="cambios()" precioreal="${precio}"
						name="nuevoPrecioProducto" value="${precio}" required>
					</div>
				</div>
				</div>
			`) */
...
function completarHTML(strongMoneda, idProducto, descripcion, stock, precio) {
	var html = `
	<div class="row" style="padding:5px 15px">
		<!-- Descripción del producto -->
		<div class="col-xs-6" style="padding-right:0px">
